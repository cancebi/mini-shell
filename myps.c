...
const char* get_color(int state) {
    switch (state) {
        case SRUN: return GREEN;  // Running
        case SSLEEP: return YELLOW; // Sleeping
        case SSTOP: return RED;   // Stopped
        case SZOMB: return RED;   // Zombie
    ...
    for (int i = 0; i < num_pids; i++) {
        ...
        if (proc_pidinfo(pids[i], PROC_PIDTBSDINFO, 0, &proc_info, sizeof(proc_info)) > 0 &&
            ...
            printf("%-10s %5d  %.1f  %.1f %8llu %8llu  ??   %c    %s  %s  %s\n",
                   ...
                   proc_info.pbi_status == SRUN ? 'R' :
                   proc_info.pbi_status == SSLEEP ? 'S' :
                   proc_info.pbi_status == SSTOP ? 'T' :
                   proc_info.pbi_status == SZOMB ? 'Z' : '?',       // STAT
                   time_buffer,                                     // STARTED
                   time_buffer,                                     // TIME
                   path_buffer                                     // COMMAND
            );
